...
void AvgCovInitCPU (uint64_t nc, const std::vector<uint64_t> &data, const Image &img) {
    Vec3 avgRes[32];
    Matrix covRes[32];
    uint64_t idx = 0;
    for (uint64_t i = 0; i < nc; ++i) {
    ...
    memcpy(avg_cpu, avgRes, 32 * sizeof(Vec3));
    memcpy(cov_cpu, covRes, 32 * sizeof(Matrix));
}
...
double getArgCPU (uint32_t color, uint64_t idx) {
    double ans = 0;
    Vec3 tmp1 = colorToVec3(color) - avg_cpu[idx], tmp2;
    initVec3(tmp2);
    for (uint64_t i = 0; i < 3; ++i) {
        for (uint64_t j = 0; j < 3; ++j) {
            tmp2[i] += tmp1[j] * cov_cpu[idx](j, i);
        }
        ans += tmp1[i] * tmp2[i];
    }
    return -ans;
...
void MahalanobisCPU (uint32_t w, uint32_t h, uint64_t nc, uint32_t *data) {
    int idx = 0;
    int offsetX = 1;

    for (uint64_t i = idx; i < w * h; i += offsetX) {
        uint32_t color = data[i];
        double max = getArgCPU(color, 0);
        uint32_t curr_class = 0;
        for (uint64_t k = 1; k < nc; ++k) {
            double tmp = getArgCPU(color, k);
            if (max < tmp) {
                max = tmp;
                curr_class = k;
            }
...
std::pair<Image, uint64_t> Image::MahalanobisDistanceGPU (const std::vector<uint64_t> &data, uint64_t nc) const {
    ...
    MahalanobisKernel<<<1, 32>>>(w, h, nc, old_buff);
    ...
    new_image.swapData();
    return std::make_pair(new_image, 0 + (uint64_t) (elapsedTime * 1000));
}

std::pair<Image, uint64_t> Image::MahalanobisDistanceCPU (const std::vector<uint64_t> &data, uint64_t nc) const {
    Image new_image(w, h, buff);
    new_image.swapData();
    uint32_t *old_buff;
    uint64_t time = 0;
    ...
    startt = std::chrono::system_clock::now();
    AvgCovInitCPU(nc, data, new_image);
    endt = std::chrono::system_clock::now();
    //time += std::chrono::duration_cast <duration_t>(endt - startt).count();

    old_buff = (uint32_t *)malloc(sizeof(uint32_t) * w * h);
    memcpy(old_buff, new_image.buff.data(), sizeof(uint32_t) * w * h);

    startt = std::chrono::system_clock::now();
    MahalanobisCPU(w, h, nc, old_buff);
    endt = std::chrono::system_clock::now();
    time += std::chrono::duration_cast <duration_t>(endt - startt).count();

    memcpy(&new_image.buff[0], old_buff, sizeof(uint32_t) * w * h);

    free(old_buff);
    endt = std::chrono::system_clock::now();
    new_image.swapData();
    return std::make_pair(new_image, time);
}
