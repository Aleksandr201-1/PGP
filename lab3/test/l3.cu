...
void CPUmahalanobis_kernel(uchar4* pixels, int w, int h, int nc) {
    for (int row = 0; row < h; row++) {
        for (int col = 0; col < w; col++) {
            ...
            std::cout << "max class: " << mIdx << " for pixel " << row << "," << col << "\n";
...
int main() {
    ...
    std::cout << std::hex << "Size: " << w << " " << h << "\n";
    for (uint32_t i = 0; i < h; ++i) {
        for (uint32_t j = 0; j < w; ++j) {
            std::swap(pixels[i * w + j].w, pixels[i * w + j].x);
            std::swap(pixels[i * w + j].y, pixels[i * w + j].z);
            uchar4 p = pixels[i * w + j];
            uint32_t x = p.x, y = p.y, z = p.z, w = p.w;
            //std::cout << x << ' ' << y << ' ' << z << ' ' << w << '\n';
            uint32_t t = (x << 24) + (y << 16) + (z << 8) + w;
            std::cout << t << ' ';
        }
        std::cout << "\n";
    }
